---
title: "Confusion Matrix for NOAA and CCI predictability of Coral Bleaching"
...
CCI_BRS05 <- CCI_BRS05 %>% 
  ...
  distinct() %>% 
  filter(year <= 2016) %>% 
  mutate(bleaching = if_else(year == "2016", "2", "0")) %>% 
  mutate(prediction = case_when(accum_DHW_12weeks < 4 ~ 0,
            accum_DHW_12weeks >= 4 & accum_DHW_12weeks < 8 ~ 1,
            accum_DHW_12weeks >= 8 ~ 2)
  )
...
CCI_BUN05A <- CCI_BUN05A %>% 
  ...
  distinct() %>%  
  filter(year <= 2016) %>% 
  mutate(bleaching = case_when(year == "2011" ~ "2",
                               year == "2016" ~ "1",
...
CCI_HAB05B <- CCI_HAB05B %>% 
  ...
  distinct() %>%  
  filter(year <= 2016) %>% 
  mutate(bleaching = if_else(year == "2011", "2", "0")) %>% 
  mutate(prediction = case_when(accum_DHW_12weeks < 4 ~ 0,
            accum_DHW_12weeks >= 4 & accum_DHW_12weeks < 8 ~ 1,
            accum_DHW_12weeks >= 8 ~ 2)
  )
...
CCI_SCOTT_RPO_1 <- CCI_SCOTT_RPO_1 %>% 
  ...
  distinct() %>%  
  filter(year <= 2016) %>% 
  mutate(bleaching = case_when(year == "1998" | year == "2016" ~ 2,
...
#combine CCI data
CCI_Combined_DHW <- rbind(CCI_BRS05, CCI_BRS07, CCI_BUN05A, CCI_Bundegi, CCI_DAR3, CCI_DARL, CCI_HAB05B, CCI_HAB10A, CCI_SCOTT_RPO_1, CCI_SCOTT_SL4, CCI_SCOTT_SS3, CCI_SCOTTNR1, CCI_SCOTTSL1, CCI_SCOTTSL2, CCI_SCOTTSL3, CCI_SCOTTSS1, CCI_SCOTTSS2, CCI_Tantabiddi, CCI_TNT, CCI_TNT07C, CCI_Wallabi_Island) %>% 
...
#combine NOAA data
NOAA_Combined_DHW <- rbind(NOAA_BRS05, NOAA_BRS07, NOAA_BUN05A, NOAA_Bundegi, NOAA_DAR3, NOAA_DARL, NOAA_HAB05B, NOAA_HAB10A, NOAA_SCOTT_RPO_1, NOAA_SCOTT_SL4, NOAA_SCOTT_SS3, NOAA_SCOTTNR1, NOAA_SCOTTSL1, NOAA_SCOTTSL2, NOAA_SCOTTSL3, NOAA_SCOTTSS1, NOAA_SCOTTSS2, NOAA_Tantabiddi, NOAA_TNT, NOAA_TNT07C, NOAA_Wallabi_Island) %>% 
  mutate(bleaching = as.character(bleaching)) %>% 
  mutate(prediction = as.character(prediction))
...
CCI_CM <- as_tibble(CCI_Combined_DHW %>% 
  group_by(Reef) %>% 
  cvms::evaluate(target_col = "bleaching", prediction_cols = "prediction", type = "multinomial") %>% 
  ungroup() 
) %>% 
  select(-c(Predictions, `Confusion Matrix`, `Class Level Results`, Process))

NOAA_CM <- as_tibble(NOAA_Combined_DHW %>%
  rename(Reef = Site) %>% 
  group_by(Reef) %>% 
  cvms::evaluate(target_col = "bleaching", prediction_cols = "prediction", type = "multinomial") %>% 
  ungroup() 
...
CCI_CM %>% 
  kbl(caption = "Confusion Matrix for predictability of bleaching using Degree Heating Week values from CCI data") %>% 
  kable_styling(full_width = TRUE) %>% 
  landscape()

NOAA_CM %>% 
  kbl(caption = "Confusion Matrix for predictability of bleaching using Degree Heating Week values from NOAA data") %>% 
  kable_styling(full_width = TRUE) %>% 
  landscape()
...
#combine CCI data
CCI_Combined_DHM <- rbind(CCI_BRS05_DHM, CCI_BRS07_DHM, CCI_BUN05A_DHM, CCI_Bundegi_DHM, CCI_DAR3_DHM, CCI_DARL_DHM, CCI_HAB05B_DHM, CCI_HAB10A_DHM, CCI_SCOTT_RPO_1_DHM, CCI_SCOTT_SL4_DHM, CCI_SCOTT_SS3_DHM, CCI_SCOTTNR1_DHM, CCI_SCOTTSL1_DHM, CCI_SCOTTSL2_DHM, CCI_SCOTTSL3_DHM, CCI_SCOTTSS1_DHM, CCI_SCOTTSS2_DHM, CCI_Tantabiddi_08TNT_DHM, CCI_Tantabiddi_13TNT_DHM, CCI_TNT_DHM, CCI_TNT07C_DHM, CCI_Wallabi_Island_DHM) %>% 
  mutate(bleaching = as.character(bleaching)) %>% 
  mutate(prediction = as.character(prediction))
...
#combine NOAA data
NOAA_Combined_DHM <- rbind(NOAA_BRS05_DHM, NOAA_BRS07_DHM, NOAA_BUN05A_DHM, NOAA_Bundegi_DHM, NOAA_DAR3_DHM, NOAA_DARL_DHM, NOAA_HAB05B_DHM, NOAA_HAB10A_DHM, NOAA_SCOTT_RPO_1_DHM, NOAA_SCOTT_SL4_DHM, NOAA_SCOTT_SS3_DHM, NOAA_SCOTTNR1_DHM, NOAA_SCOTTSL1_DHM, NOAA_SCOTTSL2_DHM, NOAA_SCOTTSL3_DHM, NOAA_SCOTTSS1_DHM, NOAA_SCOTTSS2_DHM, NOAA_Tantabiddi_08TNT_DHM, NOAA_Tantabiddi_13TNT_DHM, NOAA_TNT_DHM, NOAA_TNT07C_DHM, NOAA_Wallabi_Island_DHM) %>% 
  mutate(bleaching = as.character(bleaching)) %>% 
  mutate(prediction = as.character(prediction))
...
CCI_CM <- as_tibble(CCI_Combined_DHM %>%
  group_by(Reef) %>% 
  cvms::evaluate(target_col = "bleaching", prediction_cols = "prediction", type = "multinomial") %>% 
  ungroup() 
) %>% 
...
NOAA_CM <- as_tibble(NOAA_Combined_DHM %>%
  group_by(Reef) %>% 
  cvms::evaluate(target_col = "bleaching", prediction_cols = "prediction", type = "multinomial") %>% 
  ungroup() 
) %>% 
...
CCI_CM %>% 
  kbl(caption = "Confusion Matrix for predictability of bleaching using Degree Heating Months values from CCI data") %>% 
  kable_styling(full_width = TRUE) %>% 
  landscape()

NOAA_CM %>% 
  kbl(caption = "Confusion Matrix for predictability of bleaching using Degree Heating Months values from NOAA data") %>% 
  kable_styling(full_width = TRUE) %>% 
  landscape()
