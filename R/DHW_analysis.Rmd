...
\newpage
# BRS05, BRS07, DAR Long, TNT, 13TNT, 08TNT, Wallabi Island, Scott Reef Sites 
# Moderate and High Degree Heating Week values
...
\newpage
# **BUN05A** Yearly Sum of Moderate and High DHW Values in CCI and NOAA data

```{r DHW_Count_BUN05A, echo = FALSE, message = FALSE}
#Line Graph of yearly Moderate to High DHW values in CCI and NOAA
##BUN05A

BUN05A_Sat_DHW_Sum_Days <- BUN05A_Sat_Combined %>% 
  mutate(Year = year(Date)) %>% 
  mutate(DHW4 = if_else(accum_DHW_12weeks >= 4, 1, 0)) %>% 
  mutate(DHW8 = if_else(accum_DHW_12weeks>= 8, 1, 0)) %>% 
  mutate(DHW4 = DHW4 - DHW8) %>% #to include DHW values from 4 to <8 only
...
#Table showing maximum number of DHW >= 4 days in CCI and NOAA
Max_DHW4_CCI <- BUN05A_Sat_DHW_Sum_Days %>% 
  select(-DHW8_sum_days) %>% 
  pivot_wider(names_from = Data_Type, values_from = DHW4_sum_days) %>% 
  mutate(CCI_DHW4 = CCI) %>% 
  select(-CCI, - NOAA) %>% 
...
Max_DHW4_NOAA <- BUN05A_Sat_DHW_Sum_Days %>% 
  select(-DHW8_sum_days) %>% 
  pivot_wider(names_from = Data_Type, values_from = DHW4_sum_days) %>% 
  mutate(NOAA_DHW4 = NOAA) %>% 
  select(-CCI, - NOAA) %>% 
...
Max_DHW8_CCI <- BUN05A_Sat_DHW_Sum_Days %>% 
  select(-DHW4_sum_days) %>% 
  pivot_wider(names_from = Data_Type, values_from = DHW8_sum_days) %>% 
  mutate(CCI_DHW8 = CCI) %>% 
  select(-CCI, - NOAA) %>% 
...
Max_DHW8_NOAA <- BUN05A_Sat_DHW_Sum_Days %>% 
  select(-DHW4_sum_days) %>% 
  pivot_wider(names_from = Data_Type, values_from = DHW8_sum_days) %>% 
  mutate(NOAA_DHW8 = NOAA) %>% 
  select(-CCI, - NOAA) %>% 
...
#Visualising DHW values of >=4 and >=8
DHW4_daily <- ggplot(data = BUN05A_Sat_DHW_Sum_Days)+
  geom_line(aes(x = Year, y = DHW4_sum_days, color = Data_Type))+
  labs(y = "Count", title = "BUN05A Number of days with moderate DHW values")+
  scale_color_discrete_qualitative(name = "Satellite")
  
DHW8_daily <- ggplot(data = BUN05A_Sat_DHW_Sum_Days)+
  geom_line(aes(x = Year, y = DHW8_sum_days, color = Data_Type))+
  labs(y = "Count", title = "BUN05A Number of days with high DHW values")+
  scale_color_discrete_qualitative(name = "Satellite")
...
```
\newpage
# **Bundegi** Yearly Sum of Moderate and High DHW Values in CCI and NOAA data
```{r DHW_Count_Bundegi, echo = FALSE, message = FALSE}

#Line Graph of yearly Moderate to High DHW values in CCI and NOAA
##Bundegi

Bundegi_Sat_DHW_Sum_Days <- Bundegi_Sat_Combined %>% 
  mutate(Year = year(Date)) %>% 
  mutate(DHW4 = if_else(accum_DHW_12weeks >= 4, 1, 0)) %>% 
  mutate(DHW8 = if_else(accum_DHW_12weeks>= 8, 1, 0)) %>% 
  mutate(DHW4 = DHW4 - DHW8) %>% #to include DHW values from 4 to <8 only
...
#Table showing maximum number of DHW >= 4 days in CCI and NOAA
Max_DHW4_CCI <- Bundegi_Sat_DHW_Sum_Days %>% 
  select(-DHW8_sum_days) %>% 
  pivot_wider(names_from = Data_Type, values_from = DHW4_sum_days) %>% 
  mutate(CCI_DHW4 = CCI) %>% 
  select(-CCI, - NOAA) %>% 
...
Max_DHW4_NOAA <- Bundegi_Sat_DHW_Sum_Days %>% 
  select(-DHW8_sum_days) %>% 
  pivot_wider(names_from = Data_Type, values_from = DHW4_sum_days) %>% 
  mutate(NOAA_DHW4 = NOAA) %>% 
  select(-CCI, - NOAA) %>% 
...
Max_DHW8_CCI <- Bundegi_Sat_DHW_Sum_Days %>% 
  select(-DHW4_sum_days) %>% 
  pivot_wider(names_from = Data_Type, values_from = DHW8_sum_days) %>% 
  mutate(CCI_DHW8 = CCI) %>% 
  select(-CCI, - NOAA) %>% 
...
Max_DHW8_NOAA <- Bundegi_Sat_DHW_Sum_Days %>% 
  select(-DHW4_sum_days) %>% 
  pivot_wider(names_from = Data_Type, values_from = DHW8_sum_days) %>% 
  mutate(NOAA_DHW8 = NOAA) %>% 
  select(-CCI, - NOAA) %>% 
...
#Visualising DHW values of >=4 and >=8
DHW4_daily <- ggplot(data = Bundegi_Sat_DHW_Sum_Days)+
  geom_line(aes(x = Year, y = DHW4_sum_days, color = Data_Type))+
  labs(y = "Count", title = "Bundegi (13BND & 08BND) Number of days with moderate DHW values")+
  scale_color_discrete_qualitative(name = "Satellite")
  
DHW8_daily <- ggplot(data = Bundegi_Sat_DHW_Sum_Days)+
  geom_line(aes(x = Year, y = DHW8_sum_days, color = Data_Type))+
  labs(y = "Count", title = "Bundegi (13BND & 08BND) Number of days with high DHW values")+
  scale_color_discrete_qualitative(name = "Satellite")
