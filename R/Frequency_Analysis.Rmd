...
wave_out <- analyze.wavelet(BRS05_joint, "browse_CCI", loess.span = 0,
                            dt = 1/12,
                            dj = 1/10)
...
ggplot(tibble(Period = wave_out$Period, 
  ...
  scale_colour_discrete(name = "P-val")
...
wave_out_CCI_CCORE <- 
  analyze.coherency(BRS05_joint, 
                    ...
                    loess.span = 0, 
                    dt = 1/12, dj = 1/10)
wc.avg(wave_out_CCI_CCORE)
wc.image(wave_out_CCI_CCORE)
wt.image(wave_out_CCI_CCORE, my.series = "browse_CCI")
wt.image(wave_out_CCI_CCORE, my.series = "browse_coral_core")
...
wave_out_NOAA_CCORE <- 
  analyze.coherency(BRS05_joint, 
                    ...
                    loess.span = 0, 
                    dt = 1/12, dj = 1/10)
wc.avg(wave_out_NOAA_CCORE)
wc.image(wave_out_NOAA_CCORE)
wt.image(wave_out_NOAA_CCORE, my.series = "browse_NOAA")
wt.image(wave_out_NOAA_CCORE, my.series = "browse_coral_core")
...
par(mar = c(3, 4, 1, 2))
par(cex = 0.6)
wc.avg(wave_out_CCI_CCORE, maximum.level = 8)
wc.avg(wave_out_NOAA_CCORE, maximum.level = 8)
...
coeff <- (max(wave.avg_BRS05_Combined$power) / max(wave.avg_BRS05_CCore$power))

#transform coral core data
wave.avg_BRS05_CCore <- wave.avg_BRS05_CCore %>% 
  mutate(power = power*coeff)

wave.avg_BRS05_Combined <- rbind(wave.avg_BRS05_Combined, wave.avg_BRS05_CCore)

BRS05_Sat <- ggplot(wave.avg_BRS05_Combined) +
  geom_line(aes(x = period, y = power, color = Type)) +
  ggtitle("BRS05 NOAA, CCI and Adjusted Coral Core SST Time Series Power") +
  scale_y_continuous(
...
#setting value to transform coral core data 
coeff <- (max(wave.avg_BRS07_Combined$power) / max(wave.avg_BRS07_CCore$power))

#transform coral core data
wave.avg_BRS07_CCore <- wave.avg_BRS07_CCore %>% 
  mutate(power = power*coeff)

wave.avg_BRS07_Combined <- rbind(wave.avg_BRS07_Combined, wave.avg_BRS07_CCore)
  
BRS07_Sat <- ggplot(wave.avg_BRS07_Combined) +
  geom_line(aes(x = period, y = power, color = Type)) +
  ggtitle("BRS07 NOAA, CCI and Adjusted Coral Core SST Time Series Power") +
  scale_y_continuous(
    name = "NOAA and CCI Power",
    sec.axis = sec_axis(~./coeff, name = "Coral Core Power")
  )
